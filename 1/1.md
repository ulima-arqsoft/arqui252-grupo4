...
***Detalle:***
- Registro e inicio de sesión gestionados por Auth0, permitiendo autenticación con correo electrónico y redes sociales (ej. Google).
- Flujo seguro de recuperación de contraseña manejado por Auth0.
- Asignación de roles (Comprador o Vendedor) gestionada dentro de Auth0 y leída por el sistema.
- Panel de usuario en GameVault con historial de compras, ventas y reseñas (vinculado al perfil de Auth0).
- Gestión de reputación del vendedor mediante calificaciones.
- Configuraciones de seguridad avanzadas (como doble autenticación y bloqueo de IP) gestionadas centralmente por Auth0.
...
***Detalle:***
- Procesamiento de pagos centralizado a través del agregador Culqi, que a su vez gestiona tarjetas de crédito/débito y billeteras digitales (como Yape y Plin).
- Delegación de la validación y almacenamiento de datos financieros sensibles a Culqi (Cumplimiento PCI-DSS).
- Generación de comprobantes electrónicos (boletas/facturas).
- Control de errores en transacciones fallidas y reintentos (manejados por nuestro módulo).
- Notificación inmediata del estado del pago al usuario (recibida vía Webhooks de Culqi).
- Registro histórico de pagos y devoluciones en nuestra base de datos.
